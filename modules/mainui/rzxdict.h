...
class RzxDictNode
{
	public:
		...
		static bool remove
			( RzxDictNode<KEY, VALUE> *&root, RzxDictNode<KEY, VALUE> *&first, RzxDictNode<KEY, VALUE> *&last, KEY nkey, VALUE *&nvalue );
		static bool find(RzxDictNode<KEY, VALUE>* const &root, KEY nkey, VALUE *&nvalue );
...
class RzxDict
{
	...
	public:
		...
		bool find_nearest( KEY key, KEY &key_lower, KEY &key_higher ) const;
...
template <class KEY, class VALUE>
RzxDictNode<KEY, VALUE>::RzxDictNode( KEY nkey, VALUE *nvalue )
{
	key = nkey;
	value = nvalue;
	count = 1;
...
template <class KEY, class VALUE>
bool
RzxDictNode<KEY, VALUE>::remove
	( RzxDictNode<KEY, VALUE> *&root, RzxDictNode<KEY, VALUE> *&first,
	        RzxDictNode<KEY, VALUE> *&last, KEY nkey, VALUE *&nvalue )
{
	...
	if ( nkey < root->key )  // Deletion a gauche
	{
		...
		{
			if ( !remove
			        ( root->left, first, last, nkey, nvalue ) )
				return false;
		...
		remove
			( root->right, first, last, root->next->key, root->value );
		return true;
	}
	...
	if ( root->key < nkey )   // Deletion a droite
	{
		...
		{
			if ( !remove
			        ( root->right, first, last, nkey, nvalue ) )
				return false;
		...
		remove
			( root->left, first, last, root->prev->key, root->value );
		return true;
	}
...
template <class KEY, class VALUE>
bool
RzxDictNode<KEY, VALUE>::find(RzxDictNode<KEY, VALUE>* const&root, KEY nkey, VALUE *&nvalue )
{
...
template <class KEY, class VALUE>
bool
RzxDictNode<KEY, VALUE>::find_nearest(RzxDictNode<KEY, VALUE>* const&root, KEY nkey, KEY &nkey_lower, KEY &nkey_higher )
{
...
template <class KEY, class VALUE>
bool RzxDict<KEY, VALUE>::insert( KEY key, VALUE *value )
{
	return RzxDictNode<KEY, VALUE>::insert( root, first, last, NULL, NULL, key, value );
}

template <class KEY, class VALUE>
bool RzxDict<KEY, VALUE>::contains( KEY key ) const
{
	VALUE * val;
...
template <class KEY, class VALUE>
bool RzxDict<KEY, VALUE>::find( KEY key, VALUE *&value ) const
{
	return RzxDictNode<KEY, VALUE>::find( root, key, value );
}
template <class KEY, class VALUE>
bool RzxDict<KEY, VALUE>::find_nearest( KEY key, KEY &key_lower, KEY &key_higher ) const
{
	return RzxDictNode<KEY, VALUE>::find_nearest( root, key, key_lower, key_higher );
}

template <class KEY, class VALUE>
bool RzxDict<KEY, VALUE>::remove
	( KEY key, VALUE *&value )
{
	return RzxDictNode<KEY, VALUE>::remove
		       ( root, first, last, key, value );
}
...
template <class KEY, class VALUE>
unsigned RzxDict<KEY, VALUE>::count() const
{
	if ( root == NULL )
		return 0;
	return root->count;
