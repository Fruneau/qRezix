...
///Enregistrement de l'arriv� d'un nouveau client
...
/** Sert aussi au raffraichissement des donn�s*/
void RzxConnectionLister::login()
{
	if(displayWaiter.isEmpty()) return;
	RzxComputer *computer = displayWaiter.takeFirst();

	//Le RzxComputer peut �re nul si la machine s'est d�onnect�entre l'enregistrement de sa connexion
	...
	if(computer)
	{
		...
		// Recherche si cet ordinateur �ait d��pr�ent (refresh ou login)
...
///Enregistre la d�onnexion d'un client
...
///Retourne la liste des IP des gens connect�
/** Permet pour les plug-ins d'obtenir facilement la liste les ip connect�s */
...
///Indique si tous les serveurs sont d�onnect�
...
///Indique si on est connect��un serveur
...
///Indique �tous les modules qu'il faut rafra�hir les donn�s
...
{
	// Bo�e de dialogue non modale, pour que les comms continuent.
